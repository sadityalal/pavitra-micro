...
class SecureSessionService:
    ...
    def _ensure_cart_items_consistency(self, session_dict: dict) -> dict:
        """Ensure cart_items is always a proper dict and clean invalid entries"""
        if 'cart_items' not in session_dict or session_dict['cart_items'] is None:
            session_dict['cart_items'] = {}
        elif not isinstance(session_dict['cart_items'], dict):
            logger.warning(f"Fixing cart_items type from {type(session_dict['cart_items'])} to dict")
            session_dict['cart_items'] = {}

        # Clean up any invalid cart items
        if session_dict['cart_items']:
            valid_cart_items = {}
            for key, item in session_dict['cart_items'].items():
                if (isinstance(item, dict) and
                        'product_id' in item and
                        isinstance(item.get('quantity', 1), (int, float)) and
                        int(item['quantity']) > 0):
                    # Ensure quantity is integer
                    item['quantity'] = int(item['quantity'])
                    valid_cart_items[key] = item
                else:
                    logger.warning(f"Removing invalid cart item: {key} = {item}")
            session_dict['cart_items'] = valid_cart_items
    ...
    def create_session(self, session_data: Dict[str, Any]) -> Optional[SessionData]:
        try:
            ...
            # Ensure cart items consistency before saving
            session_dict = self._ensure_cart_items_consistency(session_dict)
        ...
        try:
            ...
            if data:
                try:
                    ...
                    # FIX: Ensure cart items consistency before processing
                    session_dict = self._ensure_cart_items_consistency(session_dict)
    ...
    def update_session_data(self, session_id: str, updates: Dict[str, Any]) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                ...
                session_dict['last_activity'] = session_dict['last_activity'].isoformat()
                session_dict['expires_at'] = session_dict['expires_at'].isoformat()
                # FIX: Ensure cart items consistency before saving
                session_dict = self._ensure_cart_items_consistency(session_dict)
                key = self._session_key(session_id)
                expiry = self.user_session_duration if session.session_type == SessionType.USER else self.guest_session_duration
                redis_success = False
                ...
                logger.error(f"❌ SessionService: Session update attempt {attempt + 1} failed: {e}")
                if attempt < max_retries - 1:
                    time.sleep(0.1)  # Small delay before retry
                else:
                    logger.error(f"❌ SessionService: All retries failed for session {session_id}")
        return False
