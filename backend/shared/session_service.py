...
class SessionService:
    def __init__(self):
        ...
        self._redis_session_prefix = "shared_session:"
        self._redis_user_session_prefix = "shared_user_session:"
        self._redis_guest_session_prefix = "shared_guest_session:"
    ...
    def create_session(self, session_data: Dict[str, Any]) -> Optional[SessionData]:
        try:
            ...
            # Store session mappings with shared prefix
            if session.user_id:
            ...
            logger.info(f"Created shared {session.session_type.value} session: {session_id}")
            return session
    ...
    def validate_session_origin(self, session: SessionData, request_ip: str, request_user_agent: str) -> bool:
        try:
            # Allow sessions from different services as long as IP is somewhat consistent
            # This is more permissive for microservices architecture
            if session.ip_address and request_ip and session.ip_address != request_ip:
                # Check if it's the same network (first 2 octets)
                session_ip_parts = session.ip_address.split('.')[:2]
                request_ip_parts = request_ip.split('.')[:2]
                if session_ip_parts != request_ip_parts:
                    logger.warning(f"Session IP network mismatch: {session.ip_address} vs {request_ip}")
                    return False
            return True
        except Exception as e:
    ...
    def delete_session(self, session_id: str) -> bool:
        try:
            ...
            logger.info(f"Deleted shared session: {session_id}")
            return True
    ...
    def validate_and_repair_session(self, session_id: str) -> bool:
        try:
            ...
            if repairs_made:
                ...
                if success:
                    logger.info(f"Successfully repaired shared session {session_id}")
                else:
                    logger.error(f"Failed to save repaired shared session {session_id}")

            return True

        except Exception as e:
            logger.error(f"Failed to validate/repair shared session {session_id}: {e}")
            return False
    ...
    def invalidate_all_user_sessions(self, user_id: int) -> bool:
        try:
            ...
            logger.info(f"Invalidated {sessions_invalidated} shared sessions for user {user_id}")
            return sessions_invalidated > 0

        except Exception as e:
            logger.error(f"Failed to invalidate all shared sessions for user {user_id}: {e}")
            return False
